# Hacknet Extension 教程

> [!WARNING]
> 本教程目前处于开发早期阶段，尚未正式发布。如需要相对完整的教程，请暂时访问 [Hacknet-Extension-Tutorial](https://github.com/FBIKdot/Hacknet-Extension-Tutorial) 或 [Old.Hacknet.wiki](https://old.hacknet.wiki)

Hacknet Extension 是 Hacknet 的玩家扩展内容，主要由媒体文件和特定格式的文本（以 XML 为主）组成。

学习 Hacknet Extension 开发所需的知识门槛较低，只需熟悉 XML 基本语法并对 Hacknet 游戏内容有大致了解即可。

本教程默认你已掌握 XML 基本语法并熟悉 Hacknet 游戏内容。若需学习 XML 基础知识，可参考 [Runoob XML 教程](https://www.runoob.com/xml/xml-tutorial.html)

教程旨在入门 Hacknet Extension 开发，对基本组成部分进行介绍。某些组成部分会包含多个功能，这些功能并不会在教程中完整罗列。在每章教程的末尾都会附带上本站的 [参考](../reference/index.md)，用于查阅完整功能列表。

对 Extension 与 Mod 的详细介绍：[FAQ: Extension 和 Mod 的区别](./hnfaq.md#difference-between-ext-and-mod)

## 参考资料

教程会主要参考以下权威内容。

官方资源：
- IntroExtension
- [Hacknet Extensions Guide (Official)](https://steamcommunity.com/sharedfiles/filedetails/?id=914587661)

游戏实现：
- Hacknet 游戏文件
- [OpenHacknet](https://github.com/UnHacknet/OpenHacknet)

## 社区资源

本教材也参考了一部分这些资源。Hacknet.wiki 还在持续更新中，在阅读本教程的过程中，如果遇到 Hacknet.wiki 没有覆盖或者没解释清楚的内容，你可以先去翻阅这些资源。

社区资源：
- [Hacknet-Extension-Tutorial](https://github.com/FBIKdot/Hacknet-Extension-Tutorial) / [Old.Hacknet.wiki](https://old.hacknet.wiki)：本站的前生
- 《Hacknet 扩展制作指南 – 入门》 by Pitiedwzr & Err0r233
- 《Hacknet 主机编辑指南》 by Cyl18
