...
BranchMissions 功能的本质是：同时加载多个 Mission，在满足某一个任务的 Goal(s) 时使用该 Mission 的 `nextMission`。
...
BranchMissions 到底指的是 Branch 还是 Branch's NextMission？为了消歧义，建议让 BranchMissions 只指分支任务这个功能。
